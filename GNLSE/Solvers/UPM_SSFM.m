function u1= UPM_SSFM(u0,dt,dz,nz,alpha,betap,gamma,tol)
...
if (length(betap) == nt)     % If the user manually specifies beta(w)
    linearoperator = linearoperator - j*betap;
...
while propagedlength < fiberlength,  
  ...
  Ef = sum(abs(ufft).^2);
  meanD = sum( j*linearoperator.*(abs(ufft).^2) ) / Ef;
  aux = ( j*linearoperator - meanD ).^2;
