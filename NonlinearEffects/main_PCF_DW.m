...
% INPUT FIELD ************************************************************
u0 = 1*(A*ni*sqrt(abs(mod.betaw(3))/mod.gamma)*sech(ni*t)); %initial field shape in W^0.5
% randn('state',0);
% u0 = wgn(nt,1,10)';
PeakPower = max(u0.^2);
...
xlabel ('t (ps)');
zlabel ('|u(z,t)|^2 (W)');
axis([-3,3,0,max(Plotdata.z),0,max(max(abs(Plotdata.u).^2))])
view(0,90);
...
axis([600,1000,0,max(Plotdata.z),0,1])
view(0,90);